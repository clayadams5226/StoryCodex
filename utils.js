...
// DEPRECATED: saveCurrentState is no longer needed
// StateManager.setState() automatically persists to Chrome storage
// Use setCurrentBook(), setCurrentItem(), setCurrentScreen() instead

// DEPRECATED: updateBook is no longer needed
// Use app.bookService.updateBook(book) instead
// This function is kept for backward compatibility with tests
// NOTE: Now uses chrome.storage.local for books (to support large images)
...
export function saveCurrentState(currentBook, currentItem, currentScreen, currentItemType) {
    ...
    // StateManager automatically persists state
    // Kept for backward compatibility with tests
    // NOTE: Now saves only references (names) to avoid sync storage size limits
    const currentState = {
        currentBookName: currentBook ? currentBook.name : null,
        currentItemName: currentItem ? (currentItem.name || currentItem.title) : null,
        currentScreen,
        currentItemType
    };
